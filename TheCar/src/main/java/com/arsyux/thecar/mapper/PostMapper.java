...
public interface PostMapper {
	...
	// 전체 게시글 전체 조회
	@Select("SELECT p.postid, p.state, p.title, p.regdate, u.username username FROM USER u, POST p WHERE u.userid = p.userid ORDER BY p.postid DESC LIMIT #{start}, #{size}")
	public List<Post> getPostList(SearchPage searchPage);
	
	// 유저이름 게시글 조회
	@Select("SELECT p.postid, p.state, p.title, p.regdate, u.username username FROM USER u, POST p WHERE u.userid = p.userid AND u.username = #{searchUsername} ORDER BY p.postid DESC LIMIT #{start}, #{size}")
	public List<Post> getPostListByUsername(SearchPage searchPage);
	
	// 제목 게시글 조회
	@Select("SELECT p.postid, p.state, p.title, p.regdate, u.username username FROM USER u, POST p WHERE u.userid = p.userid AND p.title = #{searchTitle} ORDER BY p.postid DESC LIMIT #{start}, #{size}")
	public List<Post> getPostListByTitle(SearchPage searchPage);
	
	// 내용 게시글 조회
	@Select("SELECT p.postid, p.state, p.title, p.regdate, u.username username FROM USER u, POST p WHERE u.userid = p.userid AND p.content = #{searchContent} ORDER BY p.postid DESC LIMIT #{start}, #{size}")
	public List<Post> getPostListByContent(SearchPage searchPage);
	
	// 제목내용 게시글 조회
	@Select("SELECT p.postid, p.state, p.title, p.regdate, u.username username FROM USER u, POST p WHERE u.userid = p.userid AND p.title = #{searchTitle} OR p.content = #{searchContent} ORDER BY p.postid DESC LIMIT #{start}, #{size}")
